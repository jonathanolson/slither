...
(async () => {
  ...
  const explorerCurrentPuzzleStyle: TPuzzleStyle = puzzleStyleFromProperty(
    new DerivedProperty([viewStyleModeProperty], (viewStyle) => {
  ...
  const displayTilingProperty = new LocalStorageNullableEnumerationProperty<DisplayTiling>(
    'displayTilingProperty',
    DisplayTiling.enumeration,
  ...
  const groupProperty = new DerivedProperty(
    [baseGroupProperty, highlanderModeProperty, filterModeProperty, displayTilingProperty],
    (baseGroup, highlanderMode, filterMode, displayTiling) => {
      let group = baseGroup;
      ...
      const patternBoardSet = new Set(
        displayTiling ?
          group.collection.patternBoards.filter((patternBoard) => {
      ...
      if (withoutHighlander || onlyHighlander || filterTiling || filterGeneral) {
        group = group.filterIndex((ruleIndex) => {
          ...
          if (filterMode === FilterMode.ONLY_NUMBERS_AND_EXIT_RED) {
            ...
            if (
              !features.every(
                (feature) =>
                  feature instanceof FaceFeature || (feature instanceof RedEdgeFeature && feature.edge.isExit),
              )
            ) {
              return false;
            }
  ...
  Multilink.multilink([viewStyleModeProperty], (viewStyleMode) => {
    ...
    const currentNeedsColors =
      currentMode === CollectionMode.COLOR ||
      currentMode === CollectionMode.EDGE_COLOR ||
      currentMode === CollectionMode.ALL;
    ...
    if (currentNeedsEdges && !edgesVisible) {
      ...
      if (currentNeedsColors && !colorsVisible) {
        collectionModeProperty.value =
          currentMode === CollectionMode.ALL ? CollectionMode.EDGE_SECTOR : CollectionMode.EDGE;
      }
      if (currentNeedsSectors && !sectorsVisible) {
        collectionModeProperty.value =
          currentMode === CollectionMode.ALL ? CollectionMode.EDGE_COLOR : CollectionMode.EDGE;
      }
    }
  });
  ...
  const pageIndexRangeProperty = new DerivedProperty(
    [groupProperty],
    (group) => new Range(0, Math.max(0, Math.ceil(group.size / rulesPerPage) - 1)),
  );

  const pageIndexProperty = new LocalStorageNumberProperty('pageIndexProperty', 0);

  const rulesProperty = new DerivedProperty([groupProperty, pageIndexProperty], (group, pageIndex) => {
  ...
  const viewStyleNode = new UILabeledVerticalAquaRadioButtonGroup('View Style', viewStyleModeProperty, [
    {
  ...
  const collectionRadioButtonGroup = new UILabeledVerticalAquaRadioButtonGroup('Collection', collectionModeProperty, [
    ...
    },
    {
      ...
      options: {
        enabledProperty: new DerivedProperty(
          ...
          (viewStyleMode) => viewStyleMode !== ViewStyleMode.CLASSIC && viewStyleMode !== ViewStyleMode.LINES,
        ),
      },
    },
    {
    ...
    },
    {
      ...
      options: {
        enabledProperty: new DerivedProperty(
          [viewStyleModeProperty],
          (viewStyleMode) => viewStyleMode !== ViewStyleMode.CLASSIC,
        ),
      },
    },
    {
  ...
  const previousPageButton = new ArrowButton(
    'left',
    () => {
  ...
  const nextPageButton = new ArrowButton(
    'right',
    () => {
      pageIndexProperty.value++;
    },
  ...
  const pageSlider = new Slider(
    pageIndexProperty,
    new DerivedProperty([pageIndexRangeProperty], (range) => new Range(range.min, Math.max(0.1, range.max))),
    {
  ...
  const pageNumberText = new UIText(
    new DerivedProperty(
      [pageIndexProperty, pageIndexRangeProperty],
      (pageIndex, pageIndexRange) => `Page ${pageIndex} of ${pageIndexRange.max + 1}`,
    ),
    {
      fontWeight: 'bold',
    },
  ...
  const pageNumberBox = new VBox({
    ...
    children: [
      ...
      new HBox({
        spacing: 5,
        children: [previousPageButton, pageSlider, nextPageButton],
      }),
    ],
  });
  ...
  const topBox = new HBox({
    spacing: 30,
    align: 'top',
    children: [viewStyleNode, pageNumberBox, themeNode],
  });
  ...
  Multilink.multilink(
    ...
    (rules, layoutBounds, displayTiling, viewStyleMode) => {
