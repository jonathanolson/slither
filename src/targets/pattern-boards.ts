...
(async () => {
  ...
  const getGenerationNode = (generation: TPatternBoard[], generationIndex: number): Node => {
    ...
    return new AlignBox(
      new HBox({
        spacing: 10,
        children: generation.map(
          (patternBoard, boardIndex) =>
            new VBox({
              spacing: 3,
              children: [
                alignGroup.createBox(
  ...
  console.log(
    'general pattern board length',
    generalPatternBoardGenerations.map((generations) => generations.length),
  );
