  ...
  public constructor(
    ...
    patternBoard.sectors.forEach((sector) => {
      ...
      if (sectorFeatures.length) {
        ...
        const stroke =
          isOnlyOne ? darkTheme.sectorOnlyOneColorProperty
          : isNotOne ? darkTheme.sectorNotOneColorProperty
          : isNotTwo ? darkTheme.sectorNotTwoColorProperty
          : darkTheme.sectorNotZeroColorProperty;
    ...
    patternBoard.faces.forEach((face) => {
      if (!face.isExit) {
        const faceFeature = (features.find((feature) => feature instanceof FaceFeature && feature.face === face) ??
          null) as FaceFeature | null;

        if (!faceFeature || faceFeature.value !== null) {
          const string =
            faceFeature ? faceFeature.value!.toString()
            : options.showQuestionMarks ? '?'
            : '';
          if (string.length) {
            const points = planarPatternMap.faceMap.get(face)!;
    ...
    patternBoard.edges.forEach((edge) => {
      if (edge.isExit) {
        const edgeFeatures = features.filter(
          (feature) =>
            (feature instanceof BlackEdgeFeature || feature instanceof RedEdgeFeature) && feature.edge === edge,
        ) as (BlackEdgeFeature | RedEdgeFeature)[];
    ...
    patternBoard.edges.forEach((edge) => {
      if (!edge.isExit) {
        const edgeFeatures = features.filter(
          (feature) => feature instanceof RedEdgeFeature && feature.edge === edge,
        ) as RedEdgeFeature[];
    ...
    patternBoard.edges.forEach((edge) => {
      if (!edge.isExit) {
        const edgeFeatures = features.filter(
          (feature) => feature instanceof BlackEdgeFeature && feature.edge === edge,
        ) as BlackEdgeFeature[];
    ...
    if (options.labels) {
      ...
      patternBoard.faces.forEach((face) => {
        ...
        if (isExit) {
          const delta = centroid.minus(
            face.edges[0].vertices
              .map((vertex) => planarPatternMap.vertexMap.get(vertex)!)
              .reduce((a, b) => a.plus(b))
              .timesScalar(1 / 2),
          );
