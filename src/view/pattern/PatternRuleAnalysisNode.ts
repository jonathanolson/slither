  ...
  public constructor(
    ...
    // Solutions
    {
      ...
      if (rule.highlander) {
        ...
        const widerSolutions = PatternBoardSolver.getSolutions(
          patternBoard,
          rule.inputFeatureSet.getHighlanderFeaturesArray(),
        );
        const widerRichSolutions = widerSolutions.map(
          (solution) => new RichSolution(patternBoard, binaryFeatureMap, solution, true),
        );

        const highlanderFilteredRichSolutions = Highlander.filterWithFeatureSet(
          widerRichSolutions,
        ...
        highlanderFinalRichSolutions.forEach((richSolution) => {
          const solution = filteredSolutions.find((solution) => {
            return (
              solution.length === richSolution.solution.length &&
              solution.every((edge) => richSolution.solutionSet.has(edge))
            );
          })!;
        ...
        container.addChild(
          new AlignBox(
            new HBox({
              ...
              children: [
                ...matchingSingleHighlanderSolutions.map(
                  (solution) =>
                    new PatternNode(patternBoard, solutionToDisplayFeatureSet(solution.solution), planarPatternMap, {
                      scale: solutionScale,
                    }),
                ),
              ],
                  ...
                  children: [
                    ...richSolutions.map(
                      (solution) =>
                        new PatternNode(
                          patternBoard,
                          solutionToDisplayFeatureSet(solution.solution),
                          planarPatternMap,
                          {
                            scale: solutionScale,
                          },
        ...
        container.addChild(
          new AlignBox(
            new HBox({
              ...
              preferredWidth: options.layoutWidth,
              children: [
                ...compatibleFeatureSets.map(
                  (solutionFeatureSet) =>
                    new PatternNode(patternBoard, solutionFeatureSet, planarPatternMap, {
                      scale: solutionScale,
                    }),
                ),
              ],
    ...
    // Embeddings
    {
      ...
      const embeddingNodes = DisplayTiling.enumeration.values
        .map((displayTiling) => {
      ...
      // Classic case
      {
        ...
        if (squareDisplayEmbedding) {
          const allFeaturesEdgeLike = rule.inputFeatureSet.getFeaturesArray().every((feature) => {
            return (
              feature instanceof FaceFeature || feature instanceof BlackEdgeFeature || feature instanceof RedEdgeFeature
            );
          });
