...
import { Font, ManualConstraint, Node, NodeOptions, Rectangle, RichText, RichTextOptions, Text, TextOptions, TPaint } from 'phet-lib/scenery';
...
import { SunConstants } from 'phet-lib/sun';
import assert, { assertEnabled } from '../../workarounds/assert';
import PhetcommonStringUtils from './PhetcommonStringUtils.ts';
import _ from '../../workarounds/_.ts';
import { DotUtils, Range, Vector2 } from 'phet-lib/dot';
...
export type NumberDisplayOptions = SelfOptions & Omit<NodeOptions, 'children'>;
  ...
  public constructor( numberProperty: TReadOnlyProperty<number | null>, displayRange: Range, providedOptions?: NumberDisplayOptions ) {
    ...
    if ( assert ) {
      ...
      if ( numberFormatterProvided || decimalOrValueProvided ) {
        assertEnabled() && assert( numberFormatterProvided !== decimalOrValueProvided,
    ...
    const numberFormatterProperty = new TinyProperty( options.numberFormatter ? options.numberFormatter : ( value: number ) => {
      if ( options.decimalPlaces === null ) {
        ...
        return DotUtils.toFixed( value, options.decimalPlaces );
      }
    } );

    assertEnabled() && assert( !options.hasOwnProperty( 'unitsNode' ), 'unitsNode is not a supported option' );

    // Set default alignments and validate
    assertEnabled() && assert( _.includes( ALIGN_VALUES, options.align ), `invalid align: ${options.align}` );
    if ( !options.noValueAlign ) {
      options.noValueAlign = options.align;
    }
    assertEnabled() && assert( _.includes( ALIGN_VALUES, options.noValueAlign ), `invalid noValueAlign: ${options.noValueAlign}` );
    assertEnabled() && assert( options.textOptions, 'did you accidentally set textOptions to null?' );
    ...
    const replaceValuePatternValue = ( valuePattern: string ) => {
      if ( valuePattern.includes( SunConstants.VALUE_NUMBERED_PLACEHOLDER ) ) {
        return PhetcommonStringUtils.format( valuePattern, SunConstants.VALUE_NAMED_PLACEHOLDER );
      }
      else {
        return valuePattern;
      }
    ...
    const longestStringProperty = new DerivedProperty( [
      ...
      return PhetcommonStringUtils.fillIn( valuePattern, {
    ...
    const valueText: Text | RichText = new ValueTextConstructor( valueStringProperty as unknown as any, valueTextOptions );
