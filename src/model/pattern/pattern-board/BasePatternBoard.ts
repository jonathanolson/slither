...
export class BasePatternBoard implements TPatternBoard {
  ...
  public constructor(
    ...
    const vertices = [
      ...
      ..._.range(0, descriptor.numExitVertices).map(
        (i) => new BasePatternVertex(i + descriptor.numNonExitVertices, true),
      ),
    ];
    ...
    if (descriptor.type === 'faces') {
      for (const vertexIndices of descriptor.vertexLists) {
        ...
        const face = new BasePatternFace(faces.length, false, faceVertices, faceEdges, faceSectors);
        faces.push(face);
      ...
      edges.forEach((edge) => {
        if (edge.faces.length < 2) {
          ...
          const exitFace = new BasePatternFace(faces.length, true, edge.vertices, [edge], []);
          faces.push(exitFace);
      ...
      edges.push(
        ..._.range(0, descriptor.edgeCount).map((i) => {
      ...
      sectors.push(
        ..._.range(0, descriptor.edgeCount).map((i) => {
          const sector = new BasePatternSector(i, vertex, [edges[i], edges[(i + 1) % descriptor.edgeCount]]);
