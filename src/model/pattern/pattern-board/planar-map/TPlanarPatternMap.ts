      ...
      if (!faceMap.has(face)) {
        ...
        if (false) {
          ...
          faceMap.set(face, [
            edgeVectors[1],
            Vector2.ZERO,
            middle.plus(perpendicular.times(-Math.sign(dotSum) * 0.5 * Math.sin((2 * Math.PI) / 3 / order))),
          ]);
        }
      }
    });
...
export const serializePlanarPatternMap = (map: TPlanarPatternMap): string => {
  ...
  if (assertEnabled()) {
    const deserialized = deserializePlanarPatternMap(result, {
      vertices: orderedVertices,
      edges: orderedEdges,
      sectors: orderedSectors,
      faces: orderedFaces,
      descriptor: {} as unknown as TPatternBoardDescriptor,
    });
    ...
    orderedSectors.forEach((sector) =>
      assert(vectorArrayEqual(map.sectorMap.get(sector)!, deserialized.sectorMap.get(sector)!)),
    );
...
export const deserializePlanarPatternMap = (string: string, patternBoard: TPatternBoard): TPlanarPatternMap => {
  ...
  faceData.forEach((points: any, index: number) => {
    faceMap.set(
      patternBoard.faces[index],
      points.map((point: any) => new Vector2(point[0], point[1])),
    );
  });
