...
export const getSerializedPatternBoardLibrary = (): SerializedPatternBoardLibrary => {
  return {
    standardPatternBoards: _.fromPairs( standardPatternBoards.map( patternBoard => [ patternBoard.name, serializePatternBoardDescriptor( patternBoard.descriptor ) ] ) ),
    planarPatternMaps: _.fromPairs( standardPatternBoards.map( patternBoard => [ patternBoard.name, serializePlanarPatternMap( planarPatternMaps.get( patternBoard )! ) ] ) ),

    standardSquareBoardGenerations: standardSquareBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardHexagonalBoardGenerations: standardHexagonalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardTriangularBoardGenerations: standardTriangularBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardCairoBoardGenerations: standardCairoBoardGenerations.map( generation => generation.map( board => board.name! ) ),

    standardRhombilleBoardGenerations: standardRhombilleBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardSnubSquareBoardGenerations: standardSnubSquareBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardTrihexagonalBoardGenerations: standardTrihexagonalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardFloretPentagonalBoardGenerations: standardFloretPentagonalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardDeltoidalTrihexagonalBoardGenerations: standardDeltoidalTrihexagonalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardPortugalBoardGenerations: standardPortugalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardRhombitrihexagonalBoardGenerations: standardRhombitrihexagonalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardPrismaticPentagonalBoardGenerations: standardPrismaticPentagonalBoardGenerations.map( generation => generation.map( board => board.name! ) ),
    standardElongatedTriangularBoardGenerations: standardElongatedTriangularBoardGenerations.map( generation => generation.map( board => board.name! ) ),
  };
};
...
if ( USE_SERIALIZED ) {
  ...
  for ( const patternBoardName of patternBoardNames ) {
    let patternBoard: TPatternBoard = new BasePatternBoard( deserializePatternBoardDescriptor( standardSerializedPatternBoardLibrary.standardPatternBoards[ patternBoardName ] ), patternBoardName );
    const planarPatternMap = deserializePlanarPatternMap( standardSerializedPatternBoardLibrary.planarPatternMaps[ patternBoardName ], patternBoard );
...
const getStandardPatternBoard = ( name: string, descriptor: TPatternBoardDescriptor ): TPatternBoard => {
  if ( USE_SERIALIZED ) {
    ...
    const planarPatternMap = name === 'single-edge' ? getSingleEdgePlanarPatternMap( patternBoard ) : getVertexPlanarPatternMap( patternBoard );
    registerStandardPatternBoard( patternBoard, planarPatternMap );
...
const loadGenerations = ( simpleName: string, longName: keyof SerializedPatternBoardLibrary, getGenerations: () => FacesPatternBoard[][] ): TPatternBoard[][] => {
  if ( USE_SERIALIZED ) {
    return ( standardSerializedPatternBoardLibrary[ longName ] as string[][] ).map( generation => generation.map( name => {
      const patternBoard = getStandardNamedPatternBoard( name )!;
      assertEnabled() && assert( patternBoard );
      return patternBoard;
    } ) );
...
// ORDER IMPORTANT(!)
export const standardSquareBoardGenerations = loadGenerations(
  'square',
  'standardSquareBoardGenerations',
  () => getSquareBoardGenerations( 5 )
);
export const standardHexagonalBoardGenerations = loadGenerations(
  'hexagonal',
...
export const standardCairoBoardGenerations = loadGenerations(
  'cairo',
  'standardCairoBoardGenerations',
  () => getCairoBoardGenerations( 4 )
);

export const standardRhombilleBoardGenerations = loadGenerations(
  'rhombille',
...
export const standardPortugalBoardGenerations = loadGenerations(
  'portugal',
