  ...
  const getKey = (solution: TPatternEdge[]): string => {
    ...
    while (remainingEdges.length) {
      ...
      const getNextEdge = (currentEdge: TPatternEdge): TPatternEdge => {
        // TODO: performance?
        const potentialEdges = currentEdge.vertices
          .flatMap((vertex) => vertex.edges)
          .filter((edge) => remainingEdges.includes(edge));
    ...
    return `${indeterminateEdges.map((edge) => (solution.includes(edge) ? '1' : '0'))}${sortedConnections.map(
      (connection) => {
