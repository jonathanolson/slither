          ...
          if (orderedFacesIndex === orderedFaces.length) {
            ...
            for (const [patternFace, targetFace] of faceMap) {
              ...
              for (let i = 0; i < patternFace.edges.length; i++) {
                ...
                assertEnabled() &&
                  nonExitEdgeMap.has(patternEdge) &&
                  assert(nonExitEdgeMap.get(patternEdge) === targetEdge);
            ...
            for (const exitEdge of pattern.edges.filter((edge) => edge.isExit)) {
              ...
              exitEdgeMap.set(
                exitEdge,
                targetVertex.edges.filter((targetEdge) => !nonExitTargetEdges.has(targetEdge)),
              );
            }
            ...
            for (const exitFace of pattern.faces.filter((face) => face.isExit)) {
              ...
              const exitTargetFace =
                targetEdge.faces[0] === otherTargetFace ? targetEdge.faces[1] : targetEdge.faces[0];
            ...
            for (const patternSector of pattern.sectors) {
              ...
              const targetSector = targetFace.sectors.find(
                (sector) => sector.edges.includes(targetEdgeA) && sector.edges.includes(targetEdgeB),
              )!;
            ...
            embeddings.push(
              Embedding.fromMaps(pattern, board, vertexMap, nonExitEdgeMap, exitEdgeMap, sectorMap, faceMap),
            );
            ...
            if (matchingEdgeIndex >= 0) {
              ...
              const previousPatternFace =
                patternEdge.faces[0] === patternFace ? patternEdge.faces[1] : patternEdge.faces[0];
              const previousTargetFace = faceMap.get(previousPatternFace)!;
              assertEnabled() && assert(previousTargetFace);

              const targetEdge = previousTargetFace.edges.find(
                (edge) => edge.vertices.includes(targetVertexA) && edge.vertices.includes(targetVertexB),
              )!;
              ...
              const offset =
                (targetVertexAIndex - patternVertexAIndex * direction + patternFace.vertices.length) %
                patternFace.vertices.length;
              ...
              assertEnabled() &&
                assert(
                  ...
                  'If this is not satisfied, we have disconnected components OR orderedFaces order is bad',
                );
              ...
              const candidateTargetFaces = targetVertex.faces.filter(
                (face) => !face.isExit && !faceInverseMap.has(face),
              );
              ...
              for (const targetFace of candidateTargetFaces) {
                ...
                for (const direction of [1, -1] as const) {
                  ...
                  const offset =
                    (targetVertexIndex - patternVertexIndex * direction + patternFace.vertices.length) %
                    patternFace.vertices.length;
    ...
    for (const targetVertex of board.vertices) {
      ...
      const recur = (
        ...
        if (edgeIndex === patternOrder) {
          ...
          const exitEdgeMap: Map<TPatternEdge, TPatternEdge[]> =
            isExit ?
              new Map([[patternVertex.exitEdge!, targetVertex.edges.filter((edge) => !usedTargetEdges.has(edge))]])
            : new Map();

          // Initialize with face mapping from sectors (e.g. non-exit faces)
          const faceMap = new Map(
            [...sectorMap.keys()].map((sector) => {
          ...
          if (patternVertex.sectors.length === 0) {
            ...
            for (const exitFace of pattern.faces.filter((face) => face.isExit)) {
              if (exitFace.edges.length === 1) {
                ...
                const otherPatternFace =
                  patternEdge.faces[0] === exitFace ? patternEdge.faces[1] : patternEdge.faces[0];
                const otherTargetFace = faceMap.get(otherPatternFace)!;
          ...
          embeddings.push(Embedding.fromMaps(pattern, board, vertexMap, edgeMap, exitEdgeMap, sectorMap, faceMap));
          if (abortOnFirstEmbedding) {
            return;
          }
        } else {
          ...
          for (const targetEdge of targetVertex.edges) {
            ...
            for (const patternSector of patternSectors) {
              ...
              const targetSector =
                targetVertex.sectors.find((sector) => {
                  // For 2-order vertices, we need to also filter to not get duplicate sectors (since both sectors have equivalent edges).
                  return (
                    sector.edges.includes(targetEdgeA) &&
                    sector.edges.includes(targetEdgeB) &&
                    (patternOrder > 2 || !targetSectors.includes(sector))
                  );
                }) ?? null;
              if (targetSector) {
...
class FaceMapping {
  ...
  public toDetailedString(patternFace: TPatternFace, targetFace: TPatternFace): string {
    return `FaceMapping( vertices: ${_.range(0, this.edgeCount)
      .map((i) => {
        return `${patternFace.vertices[i].index} => ${targetFace.vertices[this.mapVertexIndex(i)].index}`;
      })
      .join(', ')}, edges: ${_.range(0, this.edgeCount)
      .map((i) => {
        const patternEdge = patternFace.edges[i];
