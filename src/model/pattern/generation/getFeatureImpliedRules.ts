...
export const getFeatureImpliedRules = (
  ...
  if (highlander) {
    ...
    const tablePruner = new TablePruner(
      numAttributes,
      richSolutions.map((solution) => binaryFeatureMap.getSolutionAttributeSet(solution.solutionSet)),
    );
  ...
  NextClosure.forEachImplication(
    numAttributes,
    getClosure,
    (implication) => {
    ...
    },
    {
      ...
      logModuloCallback: (count, set, implications, seconds) => {
        console.log(
          `${count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')} [${binaryFeatureMap.getBinaryString(set)}] (${binaryFeatureMap.getIndicesString(set)}) ${implications.length} ${seconds}s`,
        );
      },
    },
  );
