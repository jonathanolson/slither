...
export class BinaryFeatureMap {
  ...
  public getSolutionAttributeSet(setSolution: Set<TPatternEdge>): SolutionAttributeSet {
    ...
    return SolutionAttributeSet.fromSolutionBinary(this.numAttributes, data, optionalData);
  }
  ...
  public getBinaryString(bits: bigint): string {
    return _.range(0, this.numAttributes)
      .map((i) => (this.bitsHaveIndex(bits, i) ? '1' : '0'))
      .join('');
  }
  ...
  public getFeaturesString(bits: bigint): string {
    return this.getBitsPrimaryFeatures(bits)
