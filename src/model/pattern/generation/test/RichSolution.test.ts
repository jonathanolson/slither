...
QUnit.module('RichSolution', () => {
  QUnit.test('Basic Edge/Sector Construction', (assert) => {
    ...
    const richSolutions = solutions.map(
      (solution) => new RichSolution(squareBoard, binaryFeatureMap, solution, solutionOptions.highlander),
    );
      ...
      for (const edge of squareBoard.edges) {
        if (!edge.isExit) {
          if (richSolution.solutionSet.has(edge)) {
            assert.ok(
              primaryFeatures.some((feature) => feature instanceof BlackEdgeFeature && feature.edge === edge),
              'has non-exit black edge',
            );
          } else {
            assert.ok(
              primaryFeatures.some((feature) => feature instanceof RedEdgeFeature && feature.edge === edge),
              'has non-exit red edge',
            );
          }
        } else {
          if (richSolution.solutionSet.has(edge)) {
            assert.ok(
              !primaryFeatures.some((feature) => feature instanceof BlackEdgeFeature && feature.edge === edge),
              'should not have black exit edge',
            );
            assert.ok(
              !primaryFeatures.some((feature) => feature instanceof RedEdgeFeature && feature.edge === edge),
              'should not have red exit edge',
            );
          } else {
            ...
            if (hasBlackOnExitVertex) {
              ...
              assert.ok(
                optionalFeatures.some((feature) => feature instanceof RedEdgeFeature && feature.edge === edge),
                'supports optional red',
              );
            }
          }
        }
