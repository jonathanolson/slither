...
export class FaceColorDualFeature implements TEmbeddableFeature {
  ...
  public toCanonicalString(): string {
    if (this.canonicalString === null) {
      ...
      const isPrimaryFirst =
        primaryIndices.length > secondaryIndices.length ||
        (primaryIndices.length === secondaryIndices.length && primaryIndices[0] < secondaryIndices[0]);
  ...
  public equals(other: TFeature): boolean {
    ...
    if (
      this.primaryFaces.length + this.secondaryFaces.length !==
      other.primaryFaces.length + other.secondaryFaces.length
    ) {
      return false;
    }
    ...
    return (
      (equalArray(this.primaryFaces, other.primaryFaces) && equalArray(this.secondaryFaces, other.secondaryFaces)) ||
  ...
  public isSubsetOf(other: TFeature): boolean {
    ...
    if (isSame) {
      return (
        this.primaryFaces.every((face) => other.primaryFaces.includes(face)) &&
        this.secondaryFaces.every((face) => other.secondaryFaces.includes(face))
      );
    } else {
      return (
        this.primaryFaces.every((face) => other.secondaryFaces.includes(face)) &&
        this.secondaryFaces.every((face) => other.primaryFaces.includes(face))
      );
    }
  }
  ...
  public union(other: FaceColorDualFeature): FaceColorDualFeature | null {
    const hasSameOverlap =
      this.primaryFaces.some((face) => other.primaryFaces.includes(face)) ||
      this.secondaryFaces.some((face) => other.secondaryFaces.includes(face));
    const hasOppositeOverlap =
      this.primaryFaces.some((face) => other.secondaryFaces.includes(face)) ||
      this.secondaryFaces.some((face) => other.primaryFaces.includes(face));
  ...
  public static areCanonicalWith(features: FaceColorDualFeature[], embeddings: Embedding[]): boolean {
    const comparable = features
      .map((feature) => feature.toCanonicalString())
      .sort()
      .join('//');
    ...
    for (const embedding of embeddings) {
      const embeddedComparable = features
        .map((feature) => {
  ...
  public static deserialize(
    serialized: TSerializedEmbeddableFeature & { type: 'face-color-dual' },
    patternBoard: TPatternBoard,
  ): FaceColorDualFeature {
  ...
  public static fromPrimarySecondaryFaces(
    primaryFaces: TPatternFace[],
    secondaryFaces: TPatternFace[],
  ): FaceColorDualFeature {
    assertEnabled() && assert(primaryFaces.length + secondaryFaces.length > 1);
    assertEnabled() && assert(primaryFaces.length);
