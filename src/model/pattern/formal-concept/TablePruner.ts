  ...
  public constructor(
    ...
    if (solutionAttributeSets.length > 350) {
      this.singleAttributeObjectsMap = _.range(0, numAttributes).map((i) => {
        return solutionAttributeSets.filter((solutionAttributeSet) =>
          hasWithOptionalAttribute(solutionAttributeSet, i),
        );
      });
      ...
      if (solutionAttributeSets.length > 1500) {
        this.doubleAttributeObjectsMap = _.range(0, numAttributes).map((i) => {
          return _.range(i + 1, numAttributes).map((j) => {
            return this.singleAttributeObjectsMap![i].filter((solutionAttributeSet) =>
              hasWithOptionalAttribute(solutionAttributeSet, j),
            );
          });
        });
        console.log(
          `double filter size: ${_.sum(this.doubleAttributeObjectsMap.map((arr) => _.sum(arr.map((subArr) => subArr.length))))}`,
        );
