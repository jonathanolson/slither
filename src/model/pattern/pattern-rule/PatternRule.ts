...
export class PatternRule {
  ...
  public getEmbeddedRules(embeddings: Embedding[]): PatternRule[] {
    ...
    return embeddings
      .map((embedding) => this.embedded(embedding.targetPatternBoard as TPatternBoard, embedding))
      .filter((rule) => rule !== null) as PatternRule[];
  }
  ...
  public isIsomorphicTo(other: PatternRule): boolean {
    ...
    if (
      !this.inputFeatureSet.hasSameShapeAs(other.inputFeatureSet) ||
      !this.outputFeatureSet.hasSameShapeAs(other.outputFeatureSet)
    ) {
      return false;
    }
    ...
    for (const automorphism of automorphisms) {
      ...
      if (embeddedRule) {
        ...
        if (
          embeddedRule.inputFeatureSet.equals(other.inputFeatureSet) &&
          embeddedRule.outputFeatureSet.equals(other.outputFeatureSet)
        ) {
          return true;
        }
      }
  ...
  public isSubsetOf(other: PatternRule): boolean {
    return (
      this.inputFeatureSet.isSubsetOf(other.inputFeatureSet) && this.outputFeatureSet.isSubsetOf(other.outputFeatureSet)
    );
  }
  ...
  public getMatchState(featureSet: FeatureSet): PatternRuleMatchState {
    ...
    if (
      inputCompatibility === FeatureCompatibility.INCOMPATIBLE ||
      inputCompatibility === FeatureCompatibility.NO_MATCH_NEEDS_FACE_VALUES
    ) {
  ...
  public isRedundant(embeddedRules: PatternRule[]): boolean {
    ...
    return this.outputFeatureSet.isSubsetOf(
      PatternRule.withRulesApplied(this.patternBoard, this.inputFeatureSet, filteredEmbeddedRules),
    );
  }
  ...
  public isCorrectSlow(): boolean {
    const inputSolutionsCount = PatternBoardSolver.countSolutions(
      this.patternBoard,
      this.inputFeatureSet.getFeaturesArray(),
    );
    const outputSolutionsCount = PatternBoardSolver.countSolutions(
      this.patternBoard,
      this.outputFeatureSet.getFeaturesArray(),
    );
  ...
  public equals(other: PatternRule): boolean {
    return (
      this.patternBoard === other.patternBoard &&
      this.inputFeatureSet.equals(other.inputFeatureSet) &&
      this.outputFeatureSet.equals(other.outputFeatureSet) &&
      this.highlander === other.highlander
    );
  }
  ...
  public getBinary(patternBoards: TPatternBoard[]): number[] {
    ...
    const outputOnlyFeatures = this.outputFeatureSet
      .getFeaturesArray()
      .filter((feature) => !inputFeatures.some((f) => f.equals(feature)));
    ...
    const addFeatures = (features: TEmbeddableFeature[]) => {
      ...
      const faceColorDualFeatures = features.filter(
  ...
  public static collectionDeserialize(
    patternBoards: TPatternBoard[],
    serialized: CollectionSerializedPatternRule,
  ): PatternRule {
  ...
  public static withRulesApplied(
    ...
    assertEnabled() &&
      assert(
        embeddedRules.every((otherRule) => otherRule.patternBoard === patternBoard),
        'embedding check',
      );
