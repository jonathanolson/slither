...
export const getPatternRuleAction = (
  ...
  for ( const feature of outputFeatures ) {
    if ( feature instanceof BlackEdgeFeature || feature instanceof RedEdgeFeature ) {
      ...
      for ( const sameColors of [ primaryFaceColors, secondaryFaceColors ] ) {
        if ( sameColors.length > 1 ) {
          for ( let i = 1; i < sameColors.length; i++ ) {
            ...
            actions.push( new FaceColorMakeSameAction( getFaceColorPointer( state, colorA ), getFaceColorPointer( state, colorB ) ) );
      ...
      if ( secondaryFaceColors.length && primaryFaceColors[ 0 ] !== secondaryOppositeFaceColors[ 0 ] ) {
        actions.push( new FaceColorMakeOppositeAction( getFaceColorPointer( state, primaryFaceColors[ 0 ] ), getFaceColorPointer( state, secondaryFaceColors[ 0 ] ) ) );
  ...
  const inputBlackEdges = ( inputFeatures.filter( feature => feature instanceof BlackEdgeFeature ) as BlackEdgeFeature[] ).map( feature => boardPatternBoard.getEdge( feature.edge ) );
  const outputBlackEdges = ( outputFeatures.filter( feature => feature instanceof BlackEdgeFeature ) as BlackEdgeFeature[] ).map( feature => boardPatternBoard.getEdge( feature.edge ) );

  const inputRedEdges = ( inputFeatures.filter( feature => feature instanceof RedEdgeFeature ) as RedEdgeFeature[] ).map( feature => boardPatternBoard.getEdge( feature.edge ) );
  const outputRedEdges = ( outputFeatures.filter( feature => feature instanceof RedEdgeFeature ) as RedEdgeFeature[] ).map( feature => boardPatternBoard.getEdge( feature.edge ) );

  const inputSectorsNotZero = ( inputFeatures.filter( feature => feature instanceof SectorNotZeroFeature ) as SectorNotZeroFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );
  const outputSectorsNotZero = ( outputFeatures.filter( feature => feature instanceof SectorNotZeroFeature ) as SectorNotZeroFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );

  const inputSectorsNotOne = ( inputFeatures.filter( feature => feature instanceof SectorNotOneFeature ) as SectorNotOneFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );
  const outputSectorsNotOne = ( outputFeatures.filter( feature => feature instanceof SectorNotOneFeature ) as SectorNotOneFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );

  const inputSectorsNotTwo = ( inputFeatures.filter( feature => feature instanceof SectorNotTwoFeature ) as SectorNotTwoFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );
  const outputSectorsNotTwo = ( outputFeatures.filter( feature => feature instanceof SectorNotTwoFeature ) as SectorNotTwoFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );

  const inputSectorsOnlyOne = ( inputFeatures.filter( feature => feature instanceof SectorOnlyOneFeature ) as SectorOnlyOneFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );
  const outputSectorsOnlyOne = ( outputFeatures.filter( feature => feature instanceof SectorOnlyOneFeature ) as SectorOnlyOneFeature[] ).map( feature => boardPatternBoard.getSector( feature.sector ) );
