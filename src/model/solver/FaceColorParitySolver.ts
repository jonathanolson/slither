        ...
        if (faceValue === null) {
          ...
          if (allExterior && this.options.solveToRed) {
            ...
            const hasNonAdjacentValuedFace = this.board.faces.some((otherFace) => {
              return (
                otherFace !== face && this.state.getFaceValue(otherFace) !== null && !adjacentFaces.includes(otherFace)
              );
            });

            if (hasNonAdjacentValuedFace) {
              return new AnnotatedAction(
                new CompositeAction(sides.map((side) => new EdgeStateSetAction(side.edge, EdgeState.RED))),
                {
          ...
          const findAction = (
            value: number,
            sides: Side[],
            partialPairsExcluded: [Side[], Side[]][],
          ): TAnnotatedAction<Data> | null => {
            ...
            const singleCounts = _.sortBy(
              dualCounts.flatMap((dualCount) => {
                ...
                if (dualCount.oppositeColor && dualCount.oppositeColorSides.size) {
                  singleCounts.push(
                    new SingleColorCount(
                      dualCount.oppositeColor,
                      dualCount.oppositeCount,
                      dualCount.oppositeColorSides,
                    ),
                  );
                }
            ...
            const getBaseAnnotation = (): FaceColorAnnotationPartial => {
              const pairMap = (pair: [Side[], Side[]]): [TEdge[], TEdge[]] => [
                pair[0].map((side) => side.edge),
                pair[1].map((side) => side.edge),
              ];
            ...
            if (isRed && this.options.solveToRed) {
              return new AnnotatedAction(
                new CompositeAction(
                  [...largestSingleCount.sides].map((side) => new EdgeStateSetAction(side.edge, EdgeState.RED)),
                ),
                {
            ...
            if (isBlack && this.options.solveToBlack) {
              return new AnnotatedAction(
                new CompositeAction(
                  [...largestSingleCount.sides].map((side) => new EdgeStateSetAction(side.edge, EdgeState.BLACK)),
                ),
                {
            ...
            if (isBalanced && this.options.solveColors) {
              ...
              const oppositeColors = _.uniq(oppositeSides.map((side) => side.color)).filter(
                (color) => color !== this.state.getOppositeFaceColor(mainColor),
              );

              // Sanity check?
              if (oppositeColors.length) {
                assertEnabled() &&
                  assert(oppositeColors.every((oppositeColor) => this.state.getFaceColors().includes(oppositeColor)));
                return new AnnotatedAction(
                  new CompositeAction(
                    oppositeColors.map(
                      (oppositeColor) =>
            ...
            if (this.options.allowPartialReduction) {
              for (const dualCount of dualCounts) {
                ...
                const filteredSides = sides.filter(
                  (side) => !removedMainSides.includes(side) && !removedOppositeSides.includes(side),
                );
                if (filteredSides.length) {
                  const action = findAction(
                    F - dualCount.size,
                    filteredSides,
                    partialPairsExcluded.concat([[removedMainSides, removedOppositeSides]]),
                  );
                  if (action) {
                    return action;
                  }
