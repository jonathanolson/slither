...
type Data = TFaceValueData & TEdgeStateData & TSimpleRegionData;
...
export class SafeEdgeToSimpleRegionSolver implements TSolver<Data, TAnnotatedAction<Data>> {
  ...
  public nextAction(): TAnnotatedAction<Data> | null {
      ...
      if (region.halfEdges.some((halfEdge) => nowClearedEdges.has(halfEdge.edge))) {
        ...
        if (halfEdgeRegions.length) {
          ...
          if (region.isSolved && halfEdgeRegions.length > 1) {
            ...
            const lastHalfEdge =
              halfEdgeRegions[halfEdgeRegions.length - 1][halfEdgeRegions[halfEdgeRegions.length - 1].length - 1];
    ...
    const attemptToAddEdge = (edge: TEdge): boolean => {
      ...
      const startVertexEdgeCount = startVertex.edges.filter(
        (vertexEdge) => this.state.getEdgeState(vertexEdge) === EdgeState.BLACK,
      ).length;
      const endVertexEdgeCount = endVertex.edges.filter(
        (vertexEdge) => this.state.getEdgeState(vertexEdge) === EdgeState.BLACK,
      ).length;
      ...
      if (startRegion && endRegion) {
        if (startRegion === endRegion) {
          if (SafeEdgeToSimpleRegionSolver.isSolvedWithAddedEdge(this.board, this.state, startRegion, edge)) {
            ...
            addRegion(
              new GeneralSimpleRegion(
                startRegion.id,
                SafeEdgeToSimpleRegionSolver.combineHalfEdgeArrays(startRegion.halfEdges, [edge.forwardHalf]),
                true,
              ),
            ); // NOTE: solved!
          } else {
          ...
          const newRegion = new GeneralSimpleRegion(
            primaryRegion.id,
            SafeEdgeToSimpleRegionSolver.combineHalfEdgeArrays(
        ...
        const newRegion = new GeneralSimpleRegion(
          endRegion.id,
          SafeEdgeToSimpleRegionSolver.combineHalfEdgeArrays(endRegion.halfEdges, [edge.forwardHalf]),
        );
    ...
    if (addedRegions.size || removedRegions.size || addedWeirdEdges.size || removedWeirdEdges.size) {
      return new AnnotatedAction(
        new GeneralSimpleRegionAction(this.board, addedRegions, removedRegions, addedWeirdEdges, removedWeirdEdges),
        {
          type: 'SimpleRegions',
        },
        this.board,
  ...
  public static isSolvedWithAddedEdge(
    board: TBoard,
    data: TFaceValueData & TEdgeStateData,
    simpleRegion: TSimpleRegion,
    edge: TEdge,
  ): boolean {
    // Sanity checks
  ...
  public static combineHalfEdgeArrays(...arrays: THalfEdge[][]): THalfEdge[] {
    ...
    for (let i = 1; i < arrays.length; i++) {
      ...
      if (result[0].start === arr[0].start) {
        result = [...arr.map((halfEdge) => halfEdge.reversed).reverse(), ...result];
        ...
        // TODO: push instead?
        result = [...result, ...arr.map((halfEdge) => halfEdge.reversed).reverse()];
      } else {
