...
export class GeneralFaceStateData implements TState<TFaceStateData> {
  ...
  public static deserializeState( board: TBoard, serializedFaceData: TSerializedFaceStateData ): GeneralFaceStateData {
    const map: Map<TFace, FaceState> = new Map( serializedFaceData.faces.map( ( serializedFaceState: { face: TSerializedFace; state: TSerializedFaceState } ) => {
      const face = deserializeFace( board, serializedFaceState.face );
      return [
        face,
        FaceState.deserialize( face, serializedFaceState.state )
      ];
    } ) );

    return new GeneralFaceStateData(
      board,
      face => {
