...
export class GeneralSimpleRegionAction implements TAction<TSimpleRegionData> {
  ...
  public isEmpty(): boolean {
    return (
      this.addedRegions.size === 0 &&
      this.removedRegions.size === 0 &&
      this.addedWeirdEdges.size === 0 &&
      this.removedWeirdEdges.size === 0
    );
  }
  ...
  public static deserializeAction(board: TBoard, serializedAction: TSerializedAction): GeneralSimpleRegionAction {
    return new GeneralSimpleRegionAction(
      board,
      new Set(
        serializedAction.addedRegions.map((serializedSimpleRegion: TSerializedSimpleRegion) =>
          GeneralSimpleRegion.deserializeSimpleRegion(board, serializedSimpleRegion),
        ),
      ),
      new Set(
        serializedAction.removedRegions.map((serializedSimpleRegion: TSerializedSimpleRegion) =>
          GeneralSimpleRegion.deserializeSimpleRegion(board, serializedSimpleRegion),
        ),
      ),
      new Set(
        serializedAction.addedWeirdEdges.map((serializedEdge: TSerializedEdge) =>
          deserializeEdge(board, serializedEdge),
        ),
      ),
      new Set(
        serializedAction.removedWeirdEdges.map((serializedEdge: TSerializedEdge) =>
          deserializeEdge(board, serializedEdge),
        ),
      ),
    );
