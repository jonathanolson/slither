      ...
      const serializePattern = ( pattern: AnnotatedPattern ) => {
        return {
          ...
          faceColorDuals: pattern.faceColorDuals.map( faceColorDual => {
            return {
              primaryFaces: faceColorDual.primaryFaces.map( face => face === null ? null : board.faces.indexOf( face ) ),
              secondaryFaces: faceColorDual.secondaryFaces.map( face => face === null ? null : board.faces.indexOf( face ) ),
            };
          } ),
        };
      };
  ...
  public static deserializeAction( board: TBoard, serializedAction: TSerializedAction ): AnnotatedAction<TCompleteData> {
    ...
    if ( annotationType === 'Pattern' ) {
      const deserializePattern = ( serializedPattern: any ): AnnotatedPattern => {
        return {
          faceValues: serializedPattern.faceValues.map( ( serializedFaceValue: { face: number | null; value: FaceValue } ) => ( {
          ...
          faceColorDuals: serializedPattern.faceColorDuals.map( ( serializedFaceColorDual: { primaryFaces: ( number | null )[]; secondaryFaces: ( number | null )[] } ) => {
            return {
              primaryFaces: serializedFaceColorDual.primaryFaces.map( ( face: number | null ) => face === null ? null : board.faces[ face ] ),
              secondaryFaces: serializedFaceColorDual.secondaryFaces.map( ( face: number | null ) => face === null ? null : board.faces[ face ] ),
            };
          } ),
        };
      };
      ...
      const embedding = Embedding.deserialize( rule.patternBoard, boardPatternBoard, serializedAction.annotation.embedding );
      ...
      const affectedEdges = new Set( serializedAction.annotation.affectedEdges.map( ( index: number ) => board.edges[ index ] ) as Set<TEdge> );
      const affectedSectors = new Set( serializedAction.annotation.affectedSectors.map( ( index: number ) => board.halfEdges[ index ] ) as Set<TSector> );
      const affectedFaces = new Set( serializedAction.annotation.affectedFaces.map( ( index: number ) => board.faces[ index ] ) as Set<TFace> );
