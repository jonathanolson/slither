...
export default class SectorState extends EnumerationValue {
  ...
  public static getWithValue(zero: boolean, one: boolean, two: boolean): SectorState {
    return SectorState.enumeration.values.find(
      (value) => value.zero === zero && value.one === one && value.two === two,
    )!;
  }
  ...
  public static trivialStates = [SectorState.NONE, SectorState.ONLY_ZERO, SectorState.ONLY_TWO, SectorState.ANY];
}

export type TSerializedSectorState = 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7;

const sectorStateMap = new Map<TSerializedSectorState, SectorState>(
  SectorState.enumeration.values.map((value: SectorState) => [value.serializedValue, value]),
);
