...
export class FaceColorMakeOppositeAction implements TAction<TFaceColorData> {
  ...
  public apply(state: TFaceColorData): void {
    ...
    const newA =
      bOpposite ? FaceColorMakeSameAction.combineFaces(a, bOpposite, state, removedFaceColors, faceChangeMap) : a;
    const newB =
      aOpposite ? FaceColorMakeSameAction.combineFaces(b, aOpposite, state, removedFaceColors, faceChangeMap) : b;
