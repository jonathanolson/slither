...
export class Net {
  ...
  public isComplete(data: TEdgeStateData): boolean {
  ...
  public findForcedLoopEdges(): Set<TEdge> {
    ...
    const getFilteredEdges = (vertex: TVertex, badEdge: TEdge) =>
      vertex.edges.filter((edge) => this.edges.has(edge) && edge !== badEdge);
  ...
  public static test(numVertices: number, numEdges: number): void {
    ...
    const expectedBridgeEdges = _.sortBy(
      allEdgesInSimplePaths.filter((edge) => allSimplePaths.every((path) => path.includes(edge))),
      (x) => (x as any).id,
    );
    ...
    console.log(
      `All edges in simple paths:\n${[...allEdgesInSimplePaths].map((edge) => `  ${(edge as any).id}`).join('\n')}`,
    );
    console.log(`Expected bridge edges:\n${expectedBridgeEdges.map((edge) => `  ${(edge as any).id}`).join('\n')}`);

    const { possibleEdges, bridges } = net.getPossibleEdgesAndBridges();

    console.log(
      `Possible edges:\n${_.sortBy(possibleEdges, (x) => (x as any).id)
        .map((edge) => `  ${(edge as any).id}`)
        .join('\n')}`,
    );
    console.log(
      `Bridge edges:\n${_.sortBy(bridges, (x) => (x as any).id)
        .map((edge) => `  ${(edge as any).id}`)
        .join('\n')}`,
    );
