...
export class Solver {
  ...
  public clauses(): Vec<Clause> { return this._clauses; }
  ...
  protected pickBranchLit(): Lit {
    ...
    return next.equals( Var.UNDEF ) ? Lit.UNDEF : Lit.valueOfVar( next.value(), this.rnd_pol ? this.drand() < 0.5 : this._polarity.get( next.value() ) );
  }
  ...
  protected analyze( confl: Clause, out_learnt: VecLit, out_btlevel: number ): number {
    ...
    if ( this.ccmin_mode === 2 ) {
      ...
      for ( i = j = 1; i < out_learnt.size(); i++ ) {
        if ( this.reason( Var.valueOf( out_learnt.get( i ).var() ) ) === Clause.CRef_Undef || !this.litRedundant( out_learnt.get( i ), abstract_level ) ) {
    ...
    while ( this._analyze_stack.size() > 0 ) {
      ...
      for ( let i = 1; i < c.size(); i++ ) {
        ...
        if ( !this._seen.get( pp.var() ) && this.level( pp.var() ) > 0 ) {
          if ( this.reason( Var.valueOf( pp.var() ) ) !== Clause.CRef_Undef && ( this.abstractLevel( pp.var() ) & abstract_levels ) !== 0 ) {
  ...
  protected solve_(): Lbool {
    ...
    while ( status === Lbool.UNDEF ) {
      const rest_base = this.luby_restart ? Solver.luby( this.restart_inc, curr_restarts ) : Math.pow( this.restart_inc, curr_restarts );
  ...
  public static readonly GARBAGE_FRAC_DEFAULT = 0.20;
