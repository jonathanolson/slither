...
class DeprecatedCDCLSATSolver {
  ...
  private checkStatus(clause: CDCLClause, A: number, B: number): [string, number, number, number] {
    let unit = 0;
    if (
      this.assignment.some((a) => a.variable === Math.abs(A) && a.value === A > 0) ||
  ...
  private verify(): boolean {
    for (const clause of this.clauses) {
      if (
        !clause.literals.some((lit) => this.assignment.some((a) => a.variable === Math.abs(lit) && a.value === lit > 0))
      ) {
        return false;
      }
    }
  ...
  private bcp(clauses: CDCLClause[], literal: number): CDCLClause[] | number {
    const newClauses = clauses
      .filter((c) => !c.literals.includes(literal))
      .map((c) => ({
...
function main() {
  ...
  if (result === 'UNSAT') {
    ...
    console.log('SAT');
    const assignment = (result as CDCLAssignment[])
      .map((a) => (a.value ? a.variable : -a.variable))
