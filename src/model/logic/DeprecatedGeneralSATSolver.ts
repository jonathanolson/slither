...
export class DeprecatedGeneralSATSolver {
  ...
  public solve() {
    while (true) {
      ...
      if (conflictClause === null) {
        ...
        const backtrackLevel = Math.max(
          ...backjumpVariables.map((v) => this.assignment.find((a) => a.variable === v)!.level),
        );
  ...
  private getNegatedLiteral(variable: number): number {
    return (
      this.isAssigned(variable) ?
        this.getAssignedValue(variable) ?
          -variable
        : variable
      : variable
    );
...
export class GeneralFormula {
  ...
  public static exactlyN(count: number, ...formulas: GeneralFormula[]): GeneralFormula {
    return GeneralFormula.and(GeneralFormula.atLeastN(count, ...formulas), GeneralFormula.atMostN(count, ...formulas));
  }
}
