...
export class HexagonalBoard extends BaseBoard<TStructure> implements TBoard {
  ...
  public constructor(
    ...
    const vertexNeighborDeltas = _.range(0, 6).map((i) =>
      axialNeighborDeltas[i].plus(axialNeighborDeltas[(i + 1) % 6]),
    );

    // vertex coordinates will be the sum of all three adjacent faces
    const getVertexLocationsFromFace = (f: Vector2): Vector2[] =>
      vertexNeighborDeltas.map((delta) => delta.plus(f.timesScalar(3)));
    ...
    const vertexLocations: Vector2[] = _.uniqWith(faceLocations.flatMap(getVertexLocationsFromFace), (a, b) =>
      a.equals(b),
    );
    ...
    const vertexDescriptors: TVertexDescriptor[] = vertexLocations.map((vertexLocation) => {
      return {
        ...
        viewCoordinates: qBasis
          .timesScalar(vertexLocation.x)
          .plus(rBasis.timesScalar(vertexLocation.y))
          .timesScalar(1 / 3),
      };
    });
    const faceDescriptors: TFaceDescriptor[] = faceLocations.map((faceLocation) => {
      return {
        ...
        vertices: getVertexLocationsFromFace(faceLocation).map((vertexLocation) => {
          const descriptor = vertexDescriptors.find((vertexDescriptor) =>
            vertexDescriptor.logicalCoordinates.equals(vertexLocation),
          );
