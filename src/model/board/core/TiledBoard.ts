...
export const penroseTilings: PenroseTiling[] = [
  penrose6,
  penrose10,
  penrose11,
  penrose13,
...
export class PenroseBoard extends PolygonalBoard implements TBoard {
  public constructor(
    public readonly penroseTiling: PenroseTiling,
    scale: number
  ) {

    // They are closed, we ignore the last point
    const thinPolygons = penroseTiling.thinShape.subpaths.filter( subpath => subpath.segments.length ).map( subpath => subpath.points.slice( 0, -1 ) );
    const thickPolygons = penroseTiling.thickShape.subpaths.filter( subpath => subpath.segments.length ).map( subpath => subpath.points.slice( 0, -1 ) );

    const polygons = [ ...thickPolygons, ...thinPolygons ];
...
export const tilingTest4 = () => {
  ...
  // They are closed, we ignore the last point
  const thinPolygons = penrose20.thinShape.subpaths.filter( subpath => subpath.segments.length ).map( subpath => subpath.points.slice( 0, -1 ) );
  const thickPolygons = penrose20.thickShape.subpaths.filter( subpath => subpath.segments.length ).map( subpath => subpath.points.slice( 0, -1 ) );

  [ ...thinPolygons, ...thickPolygons ].forEach( ( polygon, i ) => {
    container.addChild( new Path( Shape.polygon( polygon ), {
      fill: [ 'red', 'green', 'blue', 'yellow', 'purple', 'orange' ][ i % 6 ],
      opacity: 0.6
    } ) );
  } );
