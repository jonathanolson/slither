...
export const rescaleProtoDescriptor = (
  ...
  const polygonAreas = protoDescriptor.faces.map((descriptor) =>
    getArea(descriptor.vertices.map((v) => v.viewCoordinates)),
  );
  ...
  const vertexDescriptorMap = new Map(
    vertexDescriptors.map((descriptor, i) => [protoDescriptor.vertices[i], descriptor]),
  );
...
export const rescaleProtoDescriptorAverage = (
  ...
  return rescaleProtoDescriptor(protoDescriptor, (polygonalAreas) =>
    Math.sqrt(averageOutputArea / (_.sum(polygonalAreas) / polygonalAreas.length)),
  );
};
...
export const rescaleProtoDescriptorMinimum = (
  ...
  return rescaleProtoDescriptor(protoDescriptor, (polygonalAreas) =>
    Math.sqrt(minimumOutputArea / Math.min(...polygonalAreas)),
  );
};
...
export const createBoardDescriptor = (protoDescriptor: TProtoBoardDescriptor): TBoardDescriptor => {
  ...
  faceDescriptors = faceDescriptors.map((descriptor) =>
    getSignedArea(descriptor.vertices.map((v) => v.viewCoordinates)) > 0 ? descriptor : (
      {
        logicalCoordinates: descriptor.logicalCoordinates,
  ...
  const vertices = vertexDescriptors.map(
    (descriptor) => new BaseVertex<TStructure>(descriptor.logicalCoordinates, descriptor.viewCoordinates),
  );
  vertices.forEach((vertex) => vertexMap.set(vertex.logicalCoordinates, vertex));

  const faces = faceDescriptors.map(
    (descriptor) =>
      new BaseFace<TStructure>(
  ...
  const edges = edgeIdentifierLookup.canonicalIdentifiers.map(
    (identifier) => new BaseEdge<TStructure>(getVertex(identifier.start)!, getVertex(identifier.end)!),
  );
  edges.forEach((edge) =>
    edgeMap.set(edgeIdentifierLookup.lookupCanonical(edge.start.logicalCoordinates, edge.end.logicalCoordinates), edge),
  );

  const halfEdges = edgeIdentifierLookup.identifiers.map(
    (identifier) =>
      new BaseHalfEdge<TStructure>(
  ...
  halfEdges.forEach((halfEdge) =>
    halfEdgeMap.set(
      edgeIdentifierLookup.lookup(halfEdge.start.logicalCoordinates, halfEdge.end.logicalCoordinates),
      halfEdge,
    ),
  );
  ...
  faceDescriptors.forEach((faceDescriptor) => {
    ...
    const facesVertices = faceDescriptor.vertices.map(
      (vertexDescriptor) => getVertex(vertexDescriptor.logicalCoordinates)!,
    );
  ...
  while (boundaryHalfEdges.size) {
    ...
    const getNext = (halfEdge: HalfEdge) => {
      ...
      const adjacentLocations = edgeIdentifierLookup
        .getAdjacentLocations(nextStartLocation)
        .filter((location) => location !== halfEdge.start.logicalCoordinates);
