...
- Current code TODOs
  ...
  - Face Coloring UI
    - ColorDuals with same hue, but different light/dark values
      - FaceColorViewNode (or something) should separate out duals (primaryand opposite)
        - Smart combinations, try to maintain light/dark and hue?
          - Combination algorithm:
            - [see what exists now, how can we adapt] - Can we handle "combinations/splits" first before anything?
        - ColorDualView:
          - hue (just one per each)
          - :primary
          - ?:secondary
        - One ColorDualView for inside/outside (no hue)
      - 
      - 
      - Count everything in ColorDual for the "count"
      - Allow turning off edge/region view (!!!) for simplicity? This could be cool
      - [NO, color for region is helpful across lines] Consider showing only non-trivial (not adjacently connected) colorings?
    ...
    - [color per group helps this] Hover appearance for dual colors?
